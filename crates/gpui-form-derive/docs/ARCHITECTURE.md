...
2. Convert `#[gpui_form(component(...))]` into `Components` from `gpui-form-core`.
3. Use `ComponentLayout` implementations to generate:
   - `FormFields` struct (component state entities)
   - `FormComponents` constructors
4. Generate a `FormValueHolder` that normalizes optionality and validation.
5. When `inventory` is enabled, submit a `GpuiFormShape` to the registry for prototyping.
6. If Koruma is present, mirror validation metadata and optional fluent error labels.
