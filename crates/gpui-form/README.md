...
`PhoneNumberValidation::is_valid_or_empty`), while `validate_required_phone_number`
rejects empty input with `PhoneNumberValidationError::Required`; both have
`_for_country_label` variants. Implement the `PhoneCountry` trait on an
application country enum to map it to a `country::Id` and label once, then
validate with `validate_phone_number_for`.
...
The optional `serde` feature makes generated forms saveable/restorable and lets
the app know whether the user edited them. It is additive and opt-in.
...
tracking works on default features too. The facade also re-exports
...
`gpui_form::FieldPath` (re-exported from `gpui-form-core`) — so every consumer
of a form (validation, dirty tracking, focus, analytics, schema export) can
refer to fields through ONE typed value instead of ad-hoc strings.
...
`FieldPath` and the `path` module are re-exported from `gpui-form-core` as
`gpui_form::FieldPath` and `gpui_form::path`.
